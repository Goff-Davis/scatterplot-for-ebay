...
test('unsold: the half-range grows past the floor for many points', () => {
  ...
  c.renderChart(
    Array.from({ length: 12 }, (_, i) => unsold(i + 1, { id: `u${i}` })),
  );
...
test('renderChart([]) shows the placeholder and clears the status', () => {
  ...
  c.renderChart([]);
  assert.equal(
    c.document.getElementById('ebay-scatter-placeholder').style.display,
    'flex',
  );
  assert.equal(
    c.document.getElementById('ebay-scatter-status').textContent,
    '',
  );
...
test('renderChart(items) hides the placeholder and writes the count (sing/plural)', () => {
  ...
  assert.equal(
    c1.document.getElementById('ebay-scatter-placeholder').style.display,
    'none',
  );
  assert.equal(
    c1.document.getElementById('ebay-scatter-status').textContent,
    '1 item',
  );
  ...
  c2.renderChart([
    sold(5, { id: 's1' }),
    sold(6, { id: 's2', date: '2024-01-06' }),
  ]);
  assert.equal(
    c2.document.getElementById('ebay-scatter-status').textContent,
    '2 items',
  );
});
